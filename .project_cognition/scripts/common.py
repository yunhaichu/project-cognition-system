...
QUOTED_EVALUATION_RE = re.compile(
    ...
    r"看到了最新提交|提交信息|GitHub\]\[|\[GitHub\]|进度判断|我的修正判断|我现在给|分数上调|"
    r"概念验证：|个人本地 MVP：|通用长期认知治理基础设施：|第一，|第二，|第三，|第四，|结论：)"
)
EXTERNAL_COMMENTARY_RE = re.compile(
    r"(重新结论|核心判断|主要变化|最明显的进步|短板|剩余问题|下一步最值得|"
    r"这轮更新|这轮提交|这轮最|这个补丁|当前提交|代码核对|我看到的管线|我没有在本地运行)"
)
...
def classify_user_utterance_intent(text: str) -> str:
    """Classify whether a user message is an instruction or quoted material.

    User utterances remain raw evidence either way. This label only controls
    whether extracted claims may become core project cognition by default.
    """
    value = str(text or "").strip()
    if not value:
        return "direct_user_intent"
    line_count = len([line for line in value.splitlines() if line.strip()])
    has_quote_marker = bool(QUOTED_EVALUATION_RE.search(value))
    has_external_marker = bool(EXTERNAL_COMMENTARY_RE.search(value))
    has_direct_marker = bool(DIRECT_USER_INTENT_RE.search(value[:240]))
    has_long_paste_shape = line_count >= 8 or len(value) >= 1200
    has_colon_paste = bool(re.search(r"(评价|相关对话|以下|内容|他说|判断)[：:]", value[:240]))

    if (has_quote_marker or has_external_marker) and has_direct_marker and (has_long_paste_shape or has_colon_paste):
        return "mixed_request_with_quote"
    if has_quote_marker or has_external_marker or has_colon_paste:
        return "quoted_evaluation"
    if has_long_paste_shape and not has_direct_marker:
        return "external_commentary"
    return "direct_user_intent"
