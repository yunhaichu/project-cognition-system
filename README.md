...
- 一个通过评分、冲突检测、人工审查来降低认知漂移的最小闭环

### 核心原则

- 用户原话是最高权重证据。
- 真实工具结果是一等证据，但不能未审查就覆盖用户原话。
...
本仓库只应提交空状态文件和脱敏样例。真实项目的 `raw/`、`logs/`、`distilled/`、`proposals/` 可能包含私有对话、工具输出、本地路径或用户偏好，发布前必须审查。
...
- a minimal loop for reducing drift through scoring, conflict detection, and review

### Core Principles

- User utterances are the highest-weight evidence.
- Real tool results are first-class evidence, but they do not override user evidence without review.
...
- Conflicts must be recorded and reviewed, not silently overwritten.
...
Only empty state files and sanitized examples should be committed. Real `raw/`, `logs/`, `distilled/`, and `proposals/` content may contain private conversations, tool output, local paths, or user preferences. Review before publishing.
